...
function CheckoutSuccessContent() {
  ...
  useEffect(() => {
    ...
    // Stripe already tells us the result in the URL — use it immediately
    ...
    // Fallback: poll backend for status (max 10 seconds then give up)
    let attempts = 0;
      ...
      try {
        ...
        if (data.paymentStatus === 'COMPLETED') {
          ...
          if (attempts >= MAX_ATTEMPTS) {
            // Stripe said it succeeded via redirect — trust that
            setStatus('success');
            return;
          }
          setTimeout(checkStatus, 2000);
